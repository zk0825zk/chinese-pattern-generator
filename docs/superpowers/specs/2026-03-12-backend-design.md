...
- enum 约束 PatternType / AiTaskStatus / AiProvider，杜绝脏数据
...
- **注册**：校验用户名唯一性 → bcrypt hash 密码 → 创建 User → 签发 JWT
...
| 路由 | 方法 | 鉴权 | 功能 |
|------|------|------|------|
| `/api/auth/register` | POST | 无 | 注册新用户 |
| `/api/auth/login` | POST | 无 | 登录获取 JWT |
...
| `/api/ai/generate` | POST | 需要 | 发起 AI 生成任务 |
